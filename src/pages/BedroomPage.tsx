...
const BedroomPage = () => {
  ...
  const [
    ...
    setShowAlertPassAndWinCoinsGame,
  ] = useState(false);
  ...
  const decisionOnlineChosen = async (
    progressLeft: number,
  ...
  const finishGame = async () => {
    ...
    if (progressLeft >= 1 || progressRight >= 1) {
      ...
      const {decisionSide, sideChosen} = await decisionOnlineChosen(
        progressLeft,
        progressRight,
      );
      if (decisionSide) {
  ...
  const handleStateAlert = (newValue: boolean) =>
    setShowAlertMissGame(newValue);
  const handleStatePassAlert = (newValue: boolean) =>
    setShowAlertPassGame(newValue);
  const handleStatePassAndWinCoinsAlert = (newValue: boolean) =>
    setShowAlertPassAndWinCoinsGame(newValue);
  ...
  async function changeCharacter() {
    ...
    const numberIWant =
      Math.floor(Math.random() * characterListStorageNumber) + 0;
  ...
  return (
    <ImageBackground
      ...
      <View
        ...
        <View
          style={{width: layout.layout.width, flex: 0.2, flexDirection: 'row'}}>
          <Coins totalCoins={totalCoins} />
          <DoubleProgressBar
            progressLeft={progressLeft}
            leftChosen={startGame ? onlineLeftGame : null}
            rightChosen={startGame ? onlineRightGame : null}
            ...
            <View
              ...
              <Text
                style={{
                  fontSize: 20,
                  fontFamily:
                    Platform.OS === 'android' ? 'Arcade-Classic' : null,
                }}>
                GAMES: {games}
              </Text>
            </View>
        ...
        <View style={{width: 300, height: 30, alignSelf: 'center'}}>
          <Text
            ...
            BEDROOM ONLINE PUNCHES
          </Text>
        </View>
        <View
          style={{width: layout.layout.width, flex: 0.8, flexDirection: 'row'}}>
          ...
          <LeftFootPunch
            ...
            scale={scaleFootLeft}
            onPunch={
              startGame && onlineLeftGame
                ? () => onPunchFoot(true, false, true)
                : startGame
                ? () => onPunchFoot(true, false, false)
                : null
            }
            leftBlackFoot={fatImages.leftBlackFoot}
            leftWhiteFoot={
              progressLeft >= 1 ? fatImages.explosion : fatImages.leftWhiteFoot
            }
          ...
          <TextHelper
            startGame={startGame}
            left={'30%'}
            onlineGame={onlineLeftGame}
            text={'Left'}
          />
          <TextHelper
            startGame={startGame}
            left={'60%'}
            onlineGame={onlineRightGame}
            text={'Right'}
          />
          ...
          <RightFootPunch
            ...
            onPunch={
              startGame && onlineRightGame
                ? () => onPunchFoot(false, true, true)
                : startGame
                ? () => onPunchFoot(false, true, false)
                : null
            }
