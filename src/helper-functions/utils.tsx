...
export const goldenPrice = async (goldenPrice: string) => {
  ...
  switch (goldenPrice) {
    ...
    case 'goldenTrophy':
      if (
        gamesStorage >= 50 &&
        characterListStorage?.character?.length === 5 &&
        duckStorage?.duck?.length === 3
      ) {
...
export const wasteCoinsAndStoreDuck = async (
  coinsInModal: number,
  coinsCostDuck: number,
  duck: any,
) => {
...
export const spendCoinsForCharacter = async (
  coinsInModal: number,
  coinsCostCharacter: number,
  character: string,
) => {
