...
const RightFoot: React.FunctionComponent<RightFootProps> = ({
  onSwipeLeft,
  onSwipeRight,
  onGestureEvent,
  moveRightToRight,
  ...
  const spin = (degrees: string) =>
    spinValue.interpolate({inputRange: [0, 1], outputRange: ['0deg', degrees]});
  ...
  const onGestureEventFunc = (event: any) => {
    ...
    if (
      number > 0 !== event.nativeEvent.translationX > 0 ||
      number < 0 !== event.nativeEvent.translationX < 0
    ) {
      setPrimero(false);
      setSegundo(false);
  ...
  return (
    <View
      ...
      <PanGestureHandler onGestureEvent={onGestureEventFunc}>
        <Animated.View
          style={{
            marginLeft: width / 12,
